...
Senic Billing Next คือ Thai-first SaaS สำหรับ SME ไทยที่ออกเอกสารการเงิน 5 ประเภท ได้แก่ ใบเสนอราคา, ใบกำกับภาษี, ใบเสร็จรับเงิน, บิลเงินสด และใบส่งของ ระบบปัจจุบันเป็น monorepo ที่แบ่ง frontend/backend ชัดเจน

### Product Domain

- Core documents: `Quotation`, `TaxInvoice`, `Receipt`, `CashBill`, `DeliveryNote`
...
- UX direction: Thai-first copy, glassmorphism, dual theme `Warm Horizon` / `Deep Ocean`, mobile-first PWA behavior, and instant Thai/English i18n toggle
...
- New UI text must be compatible with i18next/react-i18next translation keys and support instant language switching between Thai and English.
...
- Use `senic-maestro` for complex tasks that need coordination.
- Use `/Senic New Feature` for a feature workflow.
- Use `/Senic Bug Fix` for root-cause debugging.
- Use `/Senic Code Review` for risk-first review.
