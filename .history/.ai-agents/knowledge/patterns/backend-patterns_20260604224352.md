...
- When introducing new document modules (for example `Quotation`), update enum values, prefix mapping, and seed/presentation metadata consistently across backend and frontend.
...
- Webhook handlers must treat duplicate events as normal during retries.
