...
export default function Sidebar({ currentPage, onNavigate, collapsed, onToggleCollapse }: SidebarProps) {
  ...
  return (
    <aside
      className={`layout-sidebar fixed left-2 top-0 z-40 flex flex-col h-screen transition-all duration-300 ease-in-out ${collapsed ? 'is-collapsed' : 'is-expanded'}`}
    >
      {/* Brand Logo */}
      ...
      {/* Navigation */}
      <nav className="layout-sidebar-scroll flex-1 overflow-y-auto py-4 px-2.5">
        {navItems.map((item) => {
          if ('divider' in item && item.divider) {
            if (collapsed) return <div key={item.id} className="my-4 mx-2 border-t border-white/10" />;
            return (
              <div key={item.id} className="px-3 pt-2 pb-1.5 mt-5 mb-1.5">
          ...
          return (
            <button
              ...
              className={`layout-sidebar-nav-item focus-ring relative w-full flex items-center gap-3 rounded-xl mb-1 transition-all duration-200 group ${collapsed ? 'justify-center px-0 py-2.5' : 'px-3 py-2.5'} ${isActive ? 'is-active' : ''}`}
      ...
      <div className="layout-sidebar-footer border-t border-white/10 px-3 py-3.5">
