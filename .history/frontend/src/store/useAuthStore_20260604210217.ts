  ...
  persist(
    ...
    }),
    {
      ...
      merge: (persistedState, currentState) => {
        const incoming = persistedState as Partial<AuthState> | undefined;
        const incomingUser = incoming?.user ? normalizeUser(incoming.user as User) : null;

        return {
          ...currentState,
          ...incoming,
          user: incomingUser,
          isAuthenticated: Boolean(incoming?.token && incomingUser),
        };
      },
    }
  )
